...
public class Cargo {
    ...
    public int getAnnosEnCargo() {
        ...
        if (hoy.getMes() < fechaInicio.getMes()) {
            annosEnCargo--; // todavia no paso el mes de inicio para contabilizar el año ej fecha hoy
                            // 2023/11/29 y fecha inicio 2022/12/20
        } else {
            if (hoy.getMes() == fechaInicio.getMes() && hoy.getDia() < fechaInicio.getDia()) {
                annosEnCargo--; // todaia no paso el dia del inicio en el mismo mes para contabilizar el año ej
